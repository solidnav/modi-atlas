(function(){
  ...
  function scrubTo(ms){
    if(!isFinite(ms)) return;   // ignore a bad coordinate instead of poisoning state
  ...
  // drag / click on timeline. d3-drag hands us a TouchEvent on touch devices,
  // whose clientX is undefined (the coordinate is in changedTouches) - so read
  // the touch point explicitly, otherwise the scrub math becomes NaN.
  function msFromEvent(ev){
    const rect=document.getElementById("timeline").getBoundingClientRect();
    let cx = ev && ev.clientX;
    if(cx==null && ev){
      const t=(ev.changedTouches&&ev.changedTouches[0])||(ev.touches&&ev.touches[0]);
      if(t) cx=t.clientX;
    }
    if(cx==null || !rect.width) return NaN;
    const px=(cx-rect.left)*(tlW/rect.width);
